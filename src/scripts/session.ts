...
const authLevelMap = new Map<string, number>()
const buildAuthLevelMap = (authMenus: any) => {
    authLevelMap.clear();
    authMenus.forEach((authMenu: any) => {
        if (authMenu.systemId === import.meta.env.VITE_USER_SYSTEM_ID) {
            authLevelMap.set(authMenu.menuId, authMenu.authLevel)
        }
    });
}
    ...
    if (!userSession.value) {
        ...
        if (storedSession) {
            ...
            if (storedSession.sessionId === bffSession) {
                ...
                buildAuthLevelMap(storedSession.authMenus)
...
export const setUserSession = (session: UserSession | any) => {
    ...
    buildAuthLevelMap(session.authMenus)
}
...
export const getAuthMenuLevel = (menuId?: string): number => {
    ...
    return authLevelMap.get(menuId) || 0
}
